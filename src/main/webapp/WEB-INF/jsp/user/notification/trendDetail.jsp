...
<script type='text/javascript'>
	...
	function fnSetPreNextCallback(data) {
		...
		if (data.result.pre != null) {
			$("#pre").attr("href", "/user/notification/trend/detail?trend_id=" + data.result.pre.trend_id);
			$("#pre").html(data.result.pre.title);
		}
		
		if (data.result.next != null) {
			$("#next").attr("href", "/user/notification/trend/detail?trend_id=" + data.result.next.trend_id);
...
<div id="wrap">
 <section>
    <div class="subVisual lecture">
      <div class="titleArea">
        <h2>자료실</h2>
          <span>ESS산업 및 연관산업 동향 등의 자료를 제공합니다.</span>
      </div>
    </div>
  </section>
  ...
  <section>
    <div class="subWrap">
      ...
      <div class="btnWrap">
        <a href="/user/notification/trend" class="btn viewList">목록보기</a>
      </div>
    </div>
  </section>
</div>
