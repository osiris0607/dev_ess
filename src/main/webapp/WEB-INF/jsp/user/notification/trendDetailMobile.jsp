...
<script type='text/javascript'>
	...
	function getDetail() {
		...
		comAjax.setUrl("<c:url value='/user/api/notification/trend/search/detail'/>");
	...
	function getDetailCB(data){
		...
		if (data.result.upload_file_name != null && data.result.upload_file_name != "") {
			...
			$("#attach_file_name").attr("href", "/user/api/trend/download/"+ data.result.upload_file_id);
		} 
	...
	function fnSetPreNext() {
		var comAjax = new ComAjax();
		comAjax.setUrl("<c:url value='/user/api/trend/preNext' />");
	...
	function fnSetPreNextCallback(data) {
		...
		if (data.result.pre != null) {
			$("#pre").attr("href", "/user/notification/trend/detailMobile?trend_id=" + data.result.pre.trend_id);
			$("#pre").html(data.result.pre.title);
		}
		
		if (data.result.next != null) {
			$("#next").attr("href", "/user/notification/trend/detailMobile?trend_id=" + data.result.next.trend_id);
...
<body>
  <div id="wrap">
    <header class="sub">
      <div class="headerSubWrap">
        <button class="prev" onclick="location.href='location.href='/user/mobile/main/mainMobile';"></button>
        <h1>ESS산업 동향</h1>
      </div>
    </header>
  ...
  <section>
      <div class="referenceWrap">
        ...
        <div class="btnWrap">
          <a href="/user/notification/trendMobile" class="btn viewList">목록보기</a>
        </div>
   </section>
</div>
