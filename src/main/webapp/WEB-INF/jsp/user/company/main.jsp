...
<script type='text/javascript'>
	...
	function makeMap(regionValue, companyList) {
		...
		$.each(companyList, function(key, value) {
			if (value.region == regionValue.region) {
				...
				str += "<li><a href='javascript:void(0)' onclick='detailPopup(\"" + value.company_id + "\")'>" + value.name + "</a></li>";
			}
		});
		card.append(str);
		...
		$.each(companyList, function(key, value) {
			if (value.company_id == id){
				...
				if ( gfn_isNull(value.associated_job_1) == false ) {
					str += "	<div class='relLeft'>";
					str += "		<h4>" + value.associated_job_1_name + "</h4>";
					str += "		<p>" + unescapeHtml(value.associated_job_1_definition) + "</p>";
					str += "		<a href='/user/jobDictionary/detail?job_id=" + value.associated_job_1 + "' class='btn_detail' target='_blank'>상세보기 <i class='fa fa-angle-right' aria-hidden='true'></i></a>";
					str += "	</div>";
				}

				if ( gfn_isNull(value.associated_job_2) == false ) {
					str += "	<div class='relLeft'>";
					str += "		<h4>" + value.associated_job_2_name + "</h4>";
					str += "		<p>" + unescapeHtml(value.associated_job_2_definition) + "</p>";
					str += "		<a href='/user/jobDictionary/detail?job_id=" + value.associated_job_2 + "' class='btn_detail' target='_blank'>상세보기 <i class='fa fa-angle-right' aria-hidden='true'></i></a>";
					str += "	</div>";
				}

				if ( gfn_isNull(value.associated_job_3) == false ) {
					str += "	<div class='relLeft'>";
					str += "		<h4>" + value.associated_job_3_name + "</h4>";
					str += "		<p>" + unescapeHtml(value.associated_job_3_definition) + "</p>";
					str += "		<a href='/user/jobDictionary/detail?job_id=" + value.associated_job_3 + "' class='btn_detail' target='_blank'>상세보기 <i class='fa fa-angle-right' aria-hidden='true'></i></a>";
					str += "	</div>";
				}
				
				/* str += "	<div class='relLeft'>";
				str += "		<h4>비실리콘 태양전지 소재<br />제조 작업자</h4>";
				str += "		<p>다양한 비실리콘계 태양전지 소재와 관련된 직접적인 생산 작업 수행</p>";
				str += "		<a href='' class='btn_detail'>상세보기 <i class='fa fa-angle-right' aria-hidden='true'></i></a>";
				str += "	</div>";
				str += "	<div class='relRight'>";
				str += "		<h4>비실리콘 태양전지 소재<br />제조 작업자</h4>";
				str += "		<p>다양한 비실리콘계 태양전지 소재와 관련된 직접적인 생산 작업 수행</p>";
				str += "		<a href='' class='btn_detail'>상세보기 <i class='fa fa-angle-right' aria-hidden='true'></i></a>";
				str += "	</div>"; */
	...
	function searchMap() {
		...
		comAjax.setUrl("<c:url value='/user/api/company/search/region' />");
	...
	function searchList(pageNo) {
		var comAjax = new ComAjax();
		comAjax.setUrl("<c:url value='/user/api/company/search/paging' />");
	...
	function searchListCB(data) {
		...
		if (total == 0) {
			...
			$.each(data.result, function(key, value) {
				...
				if ( gfn_isNull(value.region_name) )
				{
					str += "	<td>-</td>";
					str += "	<td>-</td>";
				}
				...
				if ( gfn_isNull(value.main_product) || value.main_product == "-" )
				{
					str += "	<td></td>";
				}
				else 
        ...
        <div class="tabs map">
          ...
          <div id="tab01" class="js-tabcontent">
            <div class="bsMap">
              ...
              <div class="bsMapWrap">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="550" height="840" viewBox="0 0 481.077 700.971">
                  <defs>
                      <style>
              ...
              <div id="Gyeongbuk" class="mapCard">
               ...
               <!--  <p class="cardTitle"><em class="en">D.</em>ESS 발전사업 &gt; <em class="en">D.1.</em> 사업개발/컨설팅</p> -->
                <div class="provinceWrap">
          ...
          <div id="tab02" class="js-tabcontent">
            <div class="bsMap pgt">
              	...
              	<table class="tbl">
                	<caption><span>ESS산업 일자리 목록 보기</span></caption>
	                <colgroup>
	                  <col style="width:10%">
	                  <col style="width:10%">
	                  <col style="width:10%">
	                ...
	                <thead>
	                  <tr>
	                    ...
	                    <th scope="col">고용인수</th>
	                    <th scope="col">주력제품/서비스</th>
	                  </tr>
	                </thead>
	                <tbody id="list_body">
	                </tbody>
...
<!-- <div id="popup" class="popup coBiz" style="display:block;"> -->
<div id="popup" class="popup coBiz">
  <div class="wrap">
    <div class="header">
      <h1>기업 정보</h1>
    </div>
    ...
    <div class="contents">
      <div class="bizTitle">
        <h2 id="company_name">삼성SDI</h2>
        <p class="summary" id="company_category">코드분류 : D. ESS 발전사업 > D.1. 사업개발/컨설팅</p>
      </div>
      <div class="bizInfo" id="company_info">
        <!-- <dl>
          ...
          <dd>삼성SDI</dd>
          <dt>표준산업분류</dt>
          <dd>(C28202)축전지 제조업</dd>
          <dt>주소</dt>
          <dd>경기도 용인시 기흥구 공세로 150-20 [17084]</dd>
          <dt>주력 제품/서비스</dt>
          <dd>PV paste (전극재료)</dd>
          <dt>대표번호</dt>
          <dd>031-8006-3100</dd>
          <dt>홈페이지</dt>
          <dd><a href="https://www.samsungsdi.co.kr" target="_blank">https://www.samsungsdi.co.kr</a></dd>
        </dl>
        <div class="relWrap">
          <h3>연관 일자리</h3>
          <div class="relLeft">
            <h4>비실리콘 태양전지 소재<br />제조 작업자</h4>
            <p>다양한 비실리콘계 태양전지 소재와 관련된 직접적인 생산 작업 수행</p>
            <a href="" class="btn_detail">상세보기 <i class="fa fa-angle-right" aria-hidden="true"></i></a>
          </div>
          <div class="relRight">
            <h4>비실리콘 태양전지 소재<br />품질 관리자</h4>
            <p>다양한 비실리콘계 태양전지 소재의 품질 관련 유지·관리·보증 업무 수행</p>
            <a href="" class="btn_detail">상세보기 <i class="fa fa-angle-right" aria-hidden="true"></i></a>
          </div>
        </div> -->
      </div>
